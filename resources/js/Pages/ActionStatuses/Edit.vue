...
const form = useForm({
  _method: "PUT",
  name: props.action_status.name,
  description: props.action_status.description,
});

const updateActionStatus = () => {
  form.post(route("action-statuses.update", props.action_status.id), {
    onFinish: () => form.reset(),
  });
};

const deleteActionStatus = () => {
  form.delete(route("action-statuses.destroy", props.action_status.id), {
    preserveScroll: true,
    onFinish: () => form.reset(),
  });
};
...
<template>
    <AppLayout title="Action Status">
       ...
       <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <div>
                <FormSection @submitted="updateActionStatus">
                    <template #title>
